...
producto1 <- ts(citas$ESTAFINAL, start=c(2013,7 ), end=c(2013,10),frequency=0)
...
producto1.train <- window(producto1, start=c(2013,7 ), end=c(2013,10),frequency=0)

# realizar una muestra de validación (Enero 2013 a Febrero 2014)
producto1.test <- window(producto1, start=c(2013, 10), end=c(2013, 11),frequency=0)
...
#modelo
modelo1 <- ets(producto1.train, model = "ZZZ", opt.crit = "mse", ic = "aic")# busca alfa delta y gama que minimiza el aic
summary(modelo1)
