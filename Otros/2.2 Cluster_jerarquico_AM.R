## Revisi?n de que los paquetes est?n instalados
...
#extraer los nombres de los pa?ses y 
#guardarlos como nombres de las filas del dataframe, los nombres de las filas quedan como el numero de la fila 
america <- america %>% 
  remove_rownames %>% 
...
#crear el cluster jer?rquico. Por defecto es complete linkage
americacomplete <- hclust(americadist)
americacomplete$height

#algunos arreglos gr?ficos
...
#unir a la base de datos para an?lisis subsiguiente
...
americafins2 %>% ggplot(aes(x=americajcluster, y=Pib_growth))+
  geom_boxplot()+
  ggtitle("Crecimiento del PIB por cl?ster")+
...
#Gr?fico de perfiles
#En jer?rquico resulta m?s ?til un heatmap
