...
carterafin2 <- carterafin %>% 
  mutate(retrasos = as.factor(retrasos),
         retrasos = factor(retrasos, ordered = TRUE,
                           levels = c("1", "0"))) #le dice que interprete como primer nivel el 1

# Estandarizaci?n de las variables escalares
...
modeloknn1 <- train(retrasos~., method = "knn",
                    tuneGrid = expand.grid(k = 1:30), #itera los valores de k
                    trControl = cross, 
                    metric = "Accuracy",
                    data = cartera.train)
modeloknn1
...
levels(cartera.train$retrasos) <- make.names(levels(factor(cartera.train$retrasos))) #vuele los nombres sintacticamente correctos para R porque o sino jode
...
# Desempe?o del modelo sintonizado en test
...
# Haciendo la partici?n de la base
set.seed(19854)
...
# Crear resultados 1 o 0 para la predicci?n de Avena
...
# Crear resultados 1 o 0 para la predicci?n de cereal
