...
ropa <- ropa %>%
  mutate(peso<-telefono*paginas*log(correo))

ropaprueba <- ropaprueba %>%
  mutate(peso<-telefono*paginas*log(correo))

ropa <- ropa %>%
  mutate(pesoSev<-servicio^2)
...
ropa <- ropa %>% 
  dplyr::select(-c(idloc, idmercado, telefono, paginas,correo, servicio,edadloc))
ropa %>% dim()
ropa %>% View()
...
ropafin <- ropafin %>%
  mutate(pesoNomTam<-log(nomina)*tamamerGrande)

ropapruebafin <- ropapruebafin %>%
  mutate(pesoNomTam<-log(nomina)*tamamerGrande)
...
ropafin2 <- ropafin %>% 
  dplyr::select(-c(tamamerPequeño,nomina,tamamerGrande, promo.3))
...
resultadoeelastic_se <- resultadoeelastic_se[,-c(2:14)]
resultadoeelastic_se$ropamujer <- resultadoeelastic_se$pred_prueba_pez
resultadoeelastic_se <- resultadoeelastic_se[,-c(2)]
write.table(resultadoeelastic_se,"submissionV2^2.csv",col.names = T, row.names = F,sep = ',',dec = '.')
