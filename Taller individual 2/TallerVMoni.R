...
ropa <- ropa %>%
  mutate(peso<-servicio*correo*paginas)

#remover el idloc, nomina, idmercado, promo
ropa <- ropa %>% 
  dplyr::select(-c(idloc, nomina,edadloc, telefono))
ropa %>% dim()
ropa %>% View()
...
ropafin2 <- ropafin %>% 
  dplyr::select(-c(tamamerPequeño,idmercado.9, idmercado.1, idmercado.2,idmercado.3,idmercado.7,idmercado.4,idmercado.6,idmercado.8, promo.3))
