...
#Pasar promo a categorica 

#ropa$promo <- factor(ropa$promo)

ropa$idmercado <- factor(ropa$idmercado)
...
idmer <- ropa%>%  
  dplyr::select(idmercado)

ropa <- ropa %>%
  mutate(peso<-telefono*paginas)
ropa <- ropa %>%
  mutate(peso1<-2*correo*telefono*2)
ropa <- ropa %>%
  mutate(peso2<-edadloc*servicio)
#remover el idloc, nomina, idmercado
ropa <- ropa %>% 
  dplyr::select(-c(idloc, nomina, impresa,promo, telefono, edadloc))
...
ropafin2 <- ropafin %>% 
  dplyr::select(-c(tamamerPequeño, idmercado.9, idmercado.1, idmercado.2,idmercado.3,idmercado.5,idmercado.4,idmercado.6,idmercado.8))
...
corrplot(predictores, method="square", tl.cex = 0.7,col=brewer.pal(n=8, name="PuOr"),addCoef.col = "black", 
         number.cex=0.7,type = "upper", diag = FALSE)
...
pred_red_prueba <- predict.glmnet(elastic8, ropaprueba, s=found8$lambda.min)
# Ridge
