...
#ropa <- ropa %>% 
#  dplyr::select(-c(idloc, nomina, idmercado, promo))
ropa %>% dim()
ropa %>% View()
...
ropapruebafin2 <- ropapruebafin %>% 
  dplyr::select(-c(tamamerPequeño, ropamujerFALSE, ropamujerTRUE))
...
# Red el?stica
pred_red <- predict.glmnet(elastic8, ropa.test, s=found8$lambda.min)
pred_red_se <- predict.glmnet(elastic8, ropa.test, s=found8$lambda.1se)
# Ridge
pred_ridge <- predict.glmnet(fitridge, ropa.test, s=foundridge$lambda.min)
pred_ridge_se <- predict.glmnet(fitridge, ropa.test, s=foundridge$lambda.1se)
# Lasso
pred_lasso <- predict.glmnet(fitlasso, ropa.test, s=foundlasso$lambda.min)
pred_lasso_se <- predict.glmnet(fitlasso, ropa.test, s=foundlasso$lambda.1se)
# Stepwise

# C?lculo  del RMSE para cada modelo
rmseelastic <- rmse(ropa.test[,1],pred_red)
rmseelastic_se <- rmse(ropa.test[,1],pred_red_se)
rmseridge <- rmse(ropa.test[,1],pred_ridge)
rmseridge_se <- rmse(ropa.test[,1],pred_ridge_se)
rmselasso <- rmse(ropa.test[,1],pred_lasso)
rmselasso_se <- rmse(ropa.test[,1],pred_lasso_se)
...
resultadoeelastic_se <- cbind(ropapruebafin,pred_step)
resultadoeelastic_se <- resultadoeelastic_se[,-c(2:15)]
